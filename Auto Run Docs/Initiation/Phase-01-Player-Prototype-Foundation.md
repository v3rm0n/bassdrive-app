...
- [ ] Run verification and fix failures until green:
  ...
  - Status note (Loop 00001, Maestro retry 3): Re-ran `flutter analyze`, `flutter test`, and `flutter run -d macos --release`; all still fail immediately with `zsh:1: command not found: flutter`, so verification remains blocked and this task stays unchecked until Flutter is installed on PATH.
